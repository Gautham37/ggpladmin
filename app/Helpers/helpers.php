...
function getDateColumnwithCreated($modelObject, $attributeName = 'updated_at')
{
    if (setting('is_human_date_format', false)) {
        $html = '<p data-toggle="tooltip" data-placement="bottom" title="${date}">${dateHuman}</p>';
    } else {
    ...
    ($modelObject['created_by']!='') ? $created = 'by '.$modelObject->createdby->name : $created='' ;
    
    $dateObj = new Carbon\Carbon($modelObject[$attributeName]);
    $replace = preg_replace('/\$\{date\}/', $dateObj->format(setting('date_format', 'l jS F Y (h:i:s)')).' '.$created, $html);
    $replace = preg_replace('/\$\{dateHuman\}/', $dateObj->diffForHumans(), $replace);
    return $replace;
}

function getDateColumnwithCreatedDays($modelObject, $attributeName = 'updated_at')
{
    if (setting('is_human_date_format', false)) {
        $html = '<p data-toggle="tooltip" data-placement="bottom" title="${date}">${dateHuman}</p>';
    } else {
    ...
    $dateObj = new Carbon\Carbon($modelObject[$attributeName]);
    $replace = preg_replace('/\$\{date\}/', $dateObj->format(setting('date_format', 'l jS F Y (h:i:s)')).' '.$created, $html);
    $replace = preg_replace('/\$\{dateHuman\}/', $dateObj->diffInDays(), $replace);
    return $replace;
}
...
function getPriceColumn($modelObject, $attributeName = 'price')
{
    ...
    if ($modelObject[$attributeName] != null && strlen($modelObject[$attributeName]) > 0) {
        ...
        if (setting('currency_right', false) != false) {
            return number_format($modelObject[$attributeName],2,'.','') . "<span>" . setting('default_currency') . "</span>";
        } else {
            return "<span>" . setting('default_currency') . "</span>" . number_format($modelObject[$attributeName],2,'.','');
        }
    }
    return '-';
}
...
function getBalanceStatus($modelObject, $attributeName = 'balance')
{
    if ($modelObject[$attributeName] != null && strlen($modelObject[$attributeName]) > 0) {
        $modelObject[$attributeName] = number_format((float)$modelObject[$attributeName], 2, '.', '');
        
        if($modelObject[$attributeName] < 0) {
           return '<span class="text-success">'.abs($modelObject[$attributeName]).' | Collectable'.'</span>';
        } else if($modelObject[$attributeName] > 0) {
            return '<span class="text-danger">'.abs($modelObject[$attributeName]).' | Payable'.'</span>';
        } else if($modelObject[$attributeName] == 0) { {
           return '<span>'.abs($modelObject[$attributeName]).' | Up to date'.'</span>';
        }
    }
...
function getBooleanColumn($column, $attributeName)
{
    if (isset($column)) {
        if ($column[$attributeName]) {
            ...
            return "<span class='btn btn-sm btn-success'>" . trans('lang.yes') . "</span>";
        } else {
           // return "<span class='badge badge-danger'>" . trans('lang.no') . "</span>";
            return "<span class='btn btn-sm btn-danger'>" . trans('lang.no') . "</span>";
        }
    }
}
 ...
 * @return string
 */
function getBooleanActiveColumn($column, $attributeName)
{
    if (isset($column)) {
        if ($column[$attributeName]) {
            //return "<span class='badge badge-success '>" . trans('lang.yes') . "</span>";
            return "<span class='btn btn-sm btn-success'>" . trans('lang.active') . "</span>";
        } else {
           // return "<span class='badge badge-danger'>" . trans('lang.no') . "</span>";
            return "<span class='btn btn-sm btn-danger'>" . trans('lang.inactive') . "</span>";
        }
    }
}
...
function getPayment($column, $attributeName)
{
    if (isset($column) && $column[$attributeName]) {
        return "<span class='btn btn-sm btn-success'>" . ucfirst($column[$attributeName]) . "</span> ";
    } else {
        return "<span class='btn btn-sm btn-danger'>" . trans('lang.order_not_paid') . "</span>";
    }
}
