<?php
/**
 * File name: ItemSalesSummaryDataTable.php
...
class ItemSalesSummaryDataTable extends DataTable
{
    /**
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ->editColumn('unit', function ($product) {
                $stock = DB::table('sales_invoice_detail')
                            ->select(DB::raw('SUM(sales_detail_quantity) as sale_quantity'))
                            ->where('sales_detail_product_id',$product['id'])
                            ->get();
                return $stock[0]->sale_quantity.' '.$product['unit'];
            })
    ...
    protected function getColumns()
    {
        $columns = [
            [
                'data' => 'name',
                'title' => strtoupper(trans('lang.stock_product_name')),
                'searchable' => true, 'orderable' => true, 'exportable' => true, 'printable' => true,
            ],
            [
                'data' => 'unit',
                'title' => strtoupper(trans('lang.product_unit').' '.trans('lang.product_quantity')),
                'searchable' => true, 'orderable' => true, 'exportable' => true, 'printable' => true,
            ]
        ];
