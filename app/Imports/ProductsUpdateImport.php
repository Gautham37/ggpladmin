...
class ProductsUpdateImport implements ToModel, WithValidation, WithHeadingRow
{
    /**
    * @param array $row
    *
    ...
    */
    ...
    {    
        ...
        $product_old = Product::where('product_code',$row['product_code'])->first();
        if($product_old) {

            $product = Product::where('product_code',$row['product_code'])->update([
            ...
            if($product) {
                ...
                if(isset($row['nutrition_list']) && count($row['nutrition_list']) > 0) {
                    foreach($row['nutrition_list'] as $key => $value) {
                        $product_nutrition = ProductNutritionDetail::firstOrCreate([
