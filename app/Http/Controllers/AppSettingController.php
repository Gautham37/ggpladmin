...
//                }
    ...
    public function index($type = null, $tab = null)
    {
        ...
        $signature = $this->uploadRepository->findByField('uuid', setting('app_invoice_signature'))->first();

        $currencies = $this->currencyRepository->all()->pluck('name_symbol', 'id');

        $customFieldModels = getModelsClasses(app_path('Models'));

        return view('settings.' . $type . '.' . $tab . '', compact(['languages', 'type', 'tab', 'langFiles', 'timezones', 'upload', 'customFieldModels', 'currencies', 'mobileLanguages', 'containsUpdate','signature']));
    }
    ...
    public function invoiceThemes() {
        $invoice_colors = DB::table('invoice_colors')->get();
        return view('invoice_themes.update')->with('invoice_colors',$invoice_colors);
    }

    public function bankDetails() {
        return view('settings.bank_details.update');
    }

    public function addressDetails() {
        return view('settings.address_details.update');
    }


    public function thermalPrint() {
        return view('thermal_print.update');
    } 
    
     public function birthdayCustomerDetails() {
        return view('settings.birthday_customer.update');
    }

    /*public function save_thermal_settings()
    {
    
     if(isset($_POST['thermal_print']))
     {
       $thermal_print = $_POST['thermal_print'];
     $app_setting = DB::table('app_settings')->where('key', 'thermal_print')->update(array('value' => $thermal_print)); 
     }
    }*/
