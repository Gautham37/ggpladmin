...
<div class="col-md-4 column custom-from-css">
    <div class="row">
        ...
        <script type="text/javascript">
            ...
            @if(isset($customer_groups) && $customer_groups->hasMedia('image'))
            var15866134771240834480ble = {
                name: "{!! $customer_groups->getFirstMedia('image')->name !!}",
                size: "{!! $customer_groups->getFirstMedia('image')->size !!}",
                type: "{!! $customer_groups->getFirstMedia('image')->mime_type !!}",
                collection_name: "{!! $customer_groups->getFirstMedia('image')->collection_name !!}"};
            @endif
            var dz_var15866134771240834480ble = $(".dropzone.image").dropzone({
                ...
                init: function () {
                @if(isset($customer_groups) && $customer_groups->hasMedia('image'))
                    dzInit(this,var15866134771240834480ble,'{!! url($customer_groups->getFirstMediaUrl('image','thumb')) !!}')
                @endif
                },
                ...
                removedfile: function (file) {
                    dzRemoveFile(
                        file, var15866134771240834480ble, '{!! url("CustomerGroups/remove-media") !!}',
                        'image', '{!! isset($customer_groups) ? $customer_groups->id : 0 !!}', '{!! url("uplaods/clear") !!}', '{!! csrf_token() !!}'
                    );
                }
            });
...
<div class="form-group col-12 text-right">
  <button type="submit" class="btn btn-{{setting('theme_color')}}" ><i class="fa fa-save"></i> {{trans('lang.save')}} {{trans('lang.customer_group_plural')}}</button>
  <a href="{!! route('CustomerGroups.index') !!}" class="btn btn-default"><i class="fa fa-undo"></i> {{trans('lang.cancel')}}</a>
</div>
