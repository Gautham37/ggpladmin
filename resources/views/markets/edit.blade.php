...
<div class="content">
  ...
  <div class="card">
    ...
    <div class="card-body">
      {!! Form::model($market, ['route' => ['markets.update', $market->id], 'method' => 'patch', 'class' => 'party-form']) !!}
      <div class="row">
...
function initialize() {
    if (stillPresent == false) {
        $('#us2').locationpicker({
            location: {
                latitude: <?php if($market->latitude!=null) { echo $market->latitude; } else { ?> $('#us2-lat').val() <?php } ?>,
                longitude: <?php if($market->longitude!=null) { echo $market->longitude; } else { ?> $('#us2-lon').val() <?php } ?>
            },
            radius: 300,
            inputBinding: {
