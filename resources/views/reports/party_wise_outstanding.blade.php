    ...
    <div class="card-body">
          ...
          @endpush
            ...
            <table class="table dataTable no-footer dtr-inline" id="dataTableBuilder" width="100%">
              <thead> 
                <tr>
                  <th>{{strtoupper(trans('lang.party_wise_outstanding_name'))}}</th>
                   <!-- <th>{{strtoupper(trans('lang.party_wise_outstanding_group'))}}</th> -->
                    <th>{{strtoupper(trans('lang.party_wise_outstanding_transactions'))}}</th>
                  <!--<th>{{strtoupper(trans('lang.party_wise_outstanding_phone_number'))}}</th>-->
                  <!--<th>{{strtoupper(trans('lang.party_wise_outstanding_contact_number'))}}</th>-->
                  <th>{{strtoupper(trans('lang.party_wise_outstanding_closing_balance'))}}</th>
                   <th>{{strtoupper(trans('lang.party_wise_outstanding_last_date'))}}</th>
                </tr>
              </thead>
              <tbody>
              </tbody>
...
<script type="text/javascript">
  ...
  var table = $('.dataTable').DataTable({
       ...
       serverSide: true,
       pageLength:20,
       ajax: "{{ route('reports.show') }}?report_type=party-wise-outstanding&start_date=&end_date=",
       columns: [
           {data: 'name', name: 'name', orderable: false, searchable: true, class: "text-left"},
           // {data: 'party_group', name: 'party_group', orderable: false, searchable: true, class: "text-left"},
           ...
           {data: 'last_transaction', name: 'last_transaction', orderable: false, searchable: true, class: "text-center"},
       ]
    });
    ...
    function cb(start, end) {
        ...
        table.ajax.url("{{route('reports.show')}}?report_type=party-wise-outstanding&start_date="+start_date+"&end_date="+end_date).load(function(result) {});
    }
    ...
    $(document).on("click","#report",function() {
        ...
        window.open("{{url('')}}/reports/exportReport?type="+type+"&report_type=party-wise-outstanding&start_date="+start_date+"&end_date="+end_date,'_blank');  
    });
