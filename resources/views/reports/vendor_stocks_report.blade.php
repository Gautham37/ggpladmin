...
<script type="text/javascript">
  ...
  var table = $('.dataTable').DataTable({
       ...
       serverSide: true,
       pageLength:20,
       ajax: "{{ route('reports.show') }}?report_type=vendor-stocks-report&product=&start_date=&end_date=",
       columns: [
    ...
    function cb(start, end) {
        ...
        table.ajax.url("{{route('reports.show')}}?report_type=vendor-stocks-report&product="+product+"&start_date="+start_date+"&end_date="+end_date).load(function(result) {});
    }
    ...
    $(document).on("click","#report",function() {
        ...
        window.open("{{url('')}}/reports/exportReport?type="+type+"&report_type=vendor-stocks-report&start_date="+start_date+"&end_date="+end_date+"&product="+product,'_blank'); 
