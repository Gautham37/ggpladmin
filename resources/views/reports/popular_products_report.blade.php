          ...
          @endpush
            ...
            <table class="table dataTable no-footer dtr-inline" id="dataTableBuilder" width="100%">
              <thead>
                ...
                <tr>
                  <th>{{strtoupper(trans('lang.popular_products_item_name'))}}</th>
                  <th>{{strtoupper(trans('lang.popular_products_item_code'))}}</th>
                  <th>{{strtoupper(trans('lang.popular_products_purchase_price'))}}</th>
                  <th>{{strtoupper(trans('lang.popular_products_selling_price'))}}</th>
                  <th>{{strtoupper(trans('lang.popular_products_stock_quantity'))}}</th>
                  <th>{{strtoupper(trans('lang.popular_products_stock_value'))}}</th>
                </tr>
              </thead>
              <tbody>
              </tbody>
...
<script type="text/javascript">
  ...
  var table = $('.dataTable').DataTable({
       ...
       serverSide: true,
       pageLength:20,
       ajax: "{{ route('reports.show') }}?report_type=popular-products-report&start_date=&end_date=",
       columns: [
    ...
    function cb(start, end) {
        ...
        table.ajax.url("{{route('reports.show')}}?report_type=popular-products-report&start_date="+start_date+"&end_date="+end_date).load(function(result) {});
    }
    ...
    $(document).on("click","#report",function() {
        ...
        window.open("{{url('')}}/reports/exportReport?type="+type+"&report_type=popular-products-report&start_date="+start_date+"&end_date="+end_date,'_blank');  
    });
