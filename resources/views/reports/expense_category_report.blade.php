    ...
    <div class="card-body">
          ...
          @endpush
            ...
            <table class="table dataTable no-footer dtr-inline" id="dataTableBuilder" width="100%">
              <thead>  
                <tr>
                  <th>{{strtoupper(trans('lang.expense_category_report_category'))}}</th>
                  <th>{{strtoupper(trans('lang.expense_category_report_total_amount'))}}</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr class="text-center">
                      <td>{{ $data->name }}</td>
                      <td>{{ $data->expense_total_amount }}</td>
                  </tr>
                  @endforeach
              </tbody>
            </table>
...
<script type="text/javascript">
  ...
  var table = $('.dataTable').DataTable({
       ...
       serverSide: true,
       pageLength:20,
       ajax: "{{ route('reports.show') }}?report_type=expenses-category-report&start_date=&end_date=",
       columns: [
    ...
    function cb(start, end) {
        ...
        table.ajax.url("{{route('reports.show')}}?report_type=expenses-category-report&start_date="+start_date+"&end_date="+end_date).load(function(result) {});
    }
    ...
    $(document).on("click","#report",function() {
        ...
        window.open("{{url('')}}/reports/exportReport?type="+type+"&report_type=expenses-category-report&start_date="+start_date+"&end_date="+end_date,'_blank');  
    });
