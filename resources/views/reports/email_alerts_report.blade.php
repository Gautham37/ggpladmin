    ...
    <div class="card-body">
          ...
          <div class="row">
            ...
            <div class="col-md-3 form-group">
              {!! Form::select('partyTypes', $partyTypes, null, ['class' => 'select2 form-control', 'id' => 'party','onchange' => 'cb(start, end)']) !!}
            </div>
...
<script type="text/javascript">
  ...
  var table = $('.dataTable').DataTable({
       ...
       serverSide: true,
       pageLength:20,
       ajax: "{{ route('reports.show') }}?report_type=email-alerts-report&party=&start_date=&end_date=",
       columns: [
           {data: 'date', name: 'date', orderable: false, searchable: true, class: "text-center"},
           {data: 'subject', name: 'subject', orderable: false, searchable: true, class: "text-center"},
           {data: 'type', name: 'type', orderable: false, searchable: true, class: "text-center"},
           {data: 'party_type_name', name: 'party_type_name', orderable: false, searchable: true, class: "text-center"},
        //   {data: 'partyname', name: 'partyname', orderable: false, searchable: true, class: "text-center"},
    ...
    function cb(start, end) {
        ...
        table.ajax.url("{{route('reports.show')}}?report_type=email-alerts-report&party="+party+"&start_date="+start_date+"&end_date="+end_date).load(function(result) {});
    }
    ...
    $(document).on("click","#report",function() {
        ...
        window.open("{{url('')}}/reports/exportReport?type="+type+"&report_type=email-alerts-report&start_date="+start_date+"&end_date="+end_date+"&party="+party,'_blank'); 
