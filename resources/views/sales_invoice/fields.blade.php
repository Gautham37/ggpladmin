    ...
    <div class="row" style="margin-top:20px;">
      ...
      @include('layouts.invoice_header')
        ...
        <div class="col-md-4 mt-4">
          ...
          <div class="form-group row">
            {!! Form::label('code', 'Sales Invoice No', ['class' => 'label-left col-md-4 control-label text-left']) !!}
            {!! Form::text('code', isset($invoice_no) ? $invoice_no : null ,  ['class' => 'col-md-8 form-control']) !!}
          </div>

          <div class="form-group row">
            {!! Form::label('date', 'Date', ['class' => 'label-left col-md-4 control-label text-left']) !!}
            {!! Form::text('date', isset($sales_invoice) ? $sales_invoice->date->format('d-m-Y') : date('d-m-Y'),  ['class' => 'col-md-8 form-control datepicker', 'readonly' => 'readonly']) !!}
          </div>

          <div class="form-group row">
            {!! Form::label('valid_date', 'Valid Date', ['class' => 'label-left col-md-4 control-label text-left']) !!}
            {!! Form::text('valid_date', isset($sales_invoice) ? $sales_invoice->valid_date->format('d-m-Y') : date('d-m-Y', time() + (86400 * 15)),  ['class' => 'col-md-8 form-control datepicker', 'readonly' => 'readonly']) !!}
          </div>
      ...
      <div class="col-md-12">
          <table id="salesItems" class="table table-bordered tbl-product-details">
            ...
            <tbody class="sales-invoice-items"></tbody>
            <!-- <tfoot>
    ...
    <div class="row">  
      ...
      <div class="col-md-7">
        ...
        <div class="col-md-12"> 
          ...
          {!! Form::textarea('terms_and_conditions', isset($sales_invoice) ? null : setting('app_invoice_terms_and_conditions'), ['class' => 'form-control']) !!}
        </div>
      </div>
...
<div class="form-group col-12 text-right">
  <button type="submit" class="btn btn-{{setting('theme_color')}} sales-invoice-form-submit" ><i class="fa fa-save"></i> Save Sales Invoice</button>
  <a href="{!! route('salesInvoice.index') !!}" class="btn btn-default"><i class="fa fa-undo"></i> Cancel </a>
</div>
...
<script>
  ...
  @if(isset($sales_invoice))
    ...
    $(document).ready(function() {
      ...
      var url   = '{!!route('salesInvoice.show',$sales_invoice->id)!!}';
      var token = "{{ csrf_token() }}";
      $.ajax({
          type: 'GET',
          data: {
  ...
  @if(!isset($sales_invoice) && isset($_GET['quote_id']) && $_GET['quote_id'] > 0)
  
    $(document).ready(function() {

      var url   = '{!!route('quotes.show',$_GET['quote_id'])!!}';
      var token = "{{ csrf_token() }}";
      $.ajax({
          type: 'GET',
          data: {
          ...
          url: url,
          success: function (response) {      
            if(response.data.status!='invoiced' && response.data.status=='accepted') { 

              $('.extra_items').append('<input type="hidden" name="quote_id" value="'+response.data.id+'" />');
              $('.market_id').val(response.data.market_id);
              $('.market_id').trigger('change');

              $('.additional_charge').val((response.data.additional_charge!=null) ? (parseFloat(response.data.additional_charge)).toFixed(2):0);
              $('.additional_charge_description').val((response.data.additional_charge_description!=null) ? response.data.additional_charge_description : '');

              $('.delivery_charge').val((response.data.delivery_charge!=null) ? (parseFloat(response.data.delivery_charge)).toFixed(2):0);
