<div class="modal-header">
  <h4 class="modal-title" id="myModalLabel">Dispose Wastage</h4>
...
{!! Form::model($inventory, ['route' => 'wastageDisposal.store', 'class' => 'wastage-disposal-form']) !!}
    ...
    <div class="row">
      <div class="col-md-12">  
        ...
        <div class="form-group text-center">
            @if($inventory->hasMedia('image'))
                <img style="width: auto; height: 150px;" src="{!! url($inventory->getFirstMediaUrl('image','medium')) !!}">
            @endif
            <h5><b>{{$product->name}}</b></h5>
        </div>

        <div class="form-group">
          {!! Form::label('type', 'Dispose To', ['class' => ' control-label text-left required']) !!}
          {!! Form::select('type', ['normal'=>'Normal','charity'=>'Charity'], null, ['class' => 'form-control']) !!}
        </div>

        <div class="form-group charity-section" style="display: none;">
          {!! Form::label('market_id', 'Charity', ['class' => ' control-label text-left required']) !!}
...
<script>
    ...
    $('#type').change(function(){
        var type = $(this).val();
        if(type=='charity') {
            $('.charity-section').show();
        } else {
            $('#market_id').val(null);
            $('.charity-section').hide();
        }
    });  
