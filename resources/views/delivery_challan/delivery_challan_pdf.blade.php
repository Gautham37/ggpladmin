      ...
      <div class="invoice_page_original" style="font-size:12px;background-color:#f4f4f533;">
         <div class="invoice_page1">
            <table style="width: auto; background-color:#f4f4f533;">
               <tr>
                  <td style="border: 1px solid #000; padding: 3px;"><b>{{ trans('lang.delivery_challan') }}</b></td>
               </tr>
            </table>
            ...
            <table style="width: 100%; line-height:15px;" class="pdf_div1" cellpadding="15">
               <tr>
                  <td width="33%">
                     <b>{{ trans('lang.dc_no') }} : </b> {{$delivery_challan->delivery_code}}
                  </td>
                  <td width="33%">
                     <b>{{ trans('lang.dc_date') }} : </b> {{date('d-m-Y',strtotime($delivery_challan->delivery_date))}}
                  </td>
                  <td width="33%">
                     <b>{{ trans('lang.po_expiry_date') }} : </b> {{date('d-m-Y',strtotime($delivery_challan->delivery_valid_date))}}
                  </td>
               </tr>
            </table>
            <div class="pdf_whole" style="padding:20px;background-color:#f4f4f533;">
               ...
               <table style="width: 100%; line-height:22px;">
                  <tr>
                     <td width="33%">  
                        ...
                        <b>{{$delivery_party->name}}</b><br>
                        <span>{{$delivery_party->address_line_1}}, {{$delivery_party->address_line_2}}<span><br>
                        <span>{{$delivery_party->city}} - {{$delivery_party->pincode}}, {{$delivery_party->state}}.<span><br>
                        @if($delivery_party->mobile!='')
                            <span>{{trans('lang.market_mobile')}} :  {{$delivery_party->mobile}}</span><br> 
                        @endif      
                        @if($delivery_party->gstin!='')               
                            <span>{{trans('lang.gstin')}} :  {{$delivery_party->gstin}}</span>  
                        @endif
                     </td>
                     <td width="33%"></td>
                     <td width="33%">
               ...
               <table cellpadding="5" style="width: 100%;border-spacing: 0px;">
                  ...
                  <?php
                     ...
                     foreach($delivery_detail as $delivery_item) {
                       ...
                       $tl_quantity[] = $delivery_item->delivery_detail_quantity;
                  ?>
                  <tr>
                     <td width="18%" align="left" {!!$sty_con!!} >{{ $delivery_item->delivery_detail_product_name }}<br>{{  $delivery_item->bar_code }}</td>
                     <td width="12%" align="center" {!!$sty_con!!} >{{$delivery_item->delivery_detail_product_hsn_code}}</td>
                     <td width="12%" align="center" {!!$sty_con!!} >{{$delivery_item->delivery_detail_quantity}} {{$delivery_item->delivery_detail_unit}}</td>
                     <td width="8%" align="center" {!!$sty_con!!} >{{number_format($delivery_item->delivery_detail_mrp,2)}}</td>
                     <td width="12%" align="center" {!!$sty_con!!} >{{number_format($delivery_item->delivery_detail_price,2)}}</td>
                     <td width="12%" align="center" {!!$sty_con!!} >
                          {{$delivery_item->delivery_detail_discount_amount}} ({{$delivery_item->delivery_detail_discount_percent}} %)
                     </td>
                     <td width="12%" align="center" {!!$sty_con!!} >
                          {{$delivery_item->delivery_detail_igst}} ({{$delivery_item->delivery_detail_tax_percent}}%)
                     </td>
                     <td width="12%" align="center" {!!$sty_con!!} >{{$delivery_item->delivery_detail_amount}}</td>
                  </tr>
                  <?php 
                     $i++;
                     } ?>
               ...
               <table class="pdf_list_original"  cellpadding="5">
                  <tr>
                     ...
                     <td width="12%" align="center" {!!$sty_pdf!!} > 
                          {!!$money_symbol!!}
                          <b>{{number_format($delivery_challan->delivery_discount_amount,2)}}</b>
                     </td>
                     <td width="12%" align="center" {!!$sty_pdf!!} >
                          {!!$money_symbol!!}
                          <b>{{number_format($delivery_challan->delivery_igst_amount,2)}}</b>
                     </td>
                     <td width="12%" align="center" {!!$sty_pdf!!} >
                          {!!$money_symbol!!}
                          <b>{{number_format($delivery_challan->delivery_total_amount - $delivery_challan->delivery_additional_charge,2)}}</b>
                     </td>
                  </tr>
                </table>
                ...
                <table style="width: 100%;">
                  ...
                  <tr>
                     ...
                     <td style="width: 15%;border:none;" align="right">
                       <b>{!!$money_symbol!!} {{number_format($delivery_challan->delivery_taxable_amount,2)}}</b>
                     </td>
                  </tr>

                  <?php if($delivery_challan->delivery_igst_amount!=0){ ?>
                  <tr>
                     <td style="width: 25%;border:none;"></td>
                     <td style="width: 35%;border:none;"></td>
                     <td style="width: 25%;border:none;" align="right">{{ trans('lang.po_sgst_amount')}}</td>
                     <td style="width: 15%;border:none;" align="right">
                          <b>{!!$money_symbol!!} {{number_format($delivery_challan->delivery_sgst_amount,2)}}</b>
                     </td>
                  </tr>
                  <tr>
                     <td style="width: 25%;border:none;"></td>
                     ...
                     <td style="width: 15%;border:none;" align="right">
                          <b>{!!$money_symbol!!} {{number_format($delivery_challan->delivery_cgst_amount,2)}}</b>
                     </td>
                  </tr>
                  <?php } ?>  

                  @if($delivery_challan->delivery_additional_charge>0)   
                  <tr>
                     <td style="width: 25%;border:none;"></td>
                     <td style="width: 35%;border:none;"></td>
                     <td style="width: 25%;" align="right"> {{$delivery_challan->delivery_additional_charge_description}}</td>
                     <td style="width: 15%;" align="right">
                          <b>{!!$money_symbol!!} {{number_format($delivery_challan->delivery_additional_charge,2)}}</b>
                     </td>
                  </tr>
                  @endif
                  <tr >
                     ...
                     <td style="width: 15%;" align="right">
                          <b>{!!$money_symbol!!} {{number_format($delivery_challan->delivery_round_off,2)}}</b>
                     </td>
                  </tr>
                  <tr>
                     <td style="width: 25%;border:none;"></td>
                     ...
                     <td style="width: 15%;border-top:2px solid #e9ecef;border-bottom:2px solid #e9ecef;" align="right">
                        <b>{!!$money_symbol!!} {{number_format(round($delivery_challan->delivery_total_amount),2)}}</b>
                     </td>
                  </tr>
                  <?php
                     //net pay per annum in words
                     $number = round($delivery_challan->delivery_total_amount);
                     $no = floor($number);
                  ...
                  <tr>
                     <td style="width: 25%;border:none;"></td>
                     <td style="width: 35%;border:none;"></td>
                     <td style="width: 35%;padding-top:10px;" align="right" colspan="2"><b style="">{{trans('lang.po_amount_in_words')}}</b></td>
                  </tr>
                  <tr >
                     <td style="width: 25%;border:none;"></td>
                     <td style="width: 35%;border:none;"></td>
                  ...
                  <!--  <tr>
                     ...
                     <td style="width: 15%;border-top:2px solid #e9ecef;" align="right"><b><span style="font-family: DejaVu Sans; sans-serif;">&#8377;</span> {{$delivery_challan->delivery_total_amount}}</b></td>
                     </tr>   --> 
               </table>
               <!-- <table class="bank_info" style="width: 100%;line-height:12px;">
                  <?php $balance_amount = 0;//$balance_amount = $grand_total - {{$delivery_challan->delivery_total_amount}}; ?>
               ...
               <table style="width: 100%;padding-top:10px;" cellpadding="5">
