         ...
         <div class="thermal_page_original">
            <table  cellpadding="5" style="font-size:14px;line-height: 18px;">
               <tbody>
                  <tr>
                     <td>
                        ...
                        {{ trans('lang.delivery_challan') }}: {{date('d-m-Y',strtotime($delivery_challan->delivery_date))}}<br>
                        {{ trans('lang.delivery_challan') }}: {{$delivery_challan->delivery_code}}<br>
             ...
             <table class="thermal_list_2inch_original"  cellpadding="5">
               ...
               <tr>
                  <td  align="left">{{$delivery_item->delivery_detail_product_name}}<br>{{number_format($delivery_item->delivery_detail_mrp,2)}}<br>{{number_format($delivery_item->delivery_detail_igst,2)}}
                  </td>
                  <td  align="left">{{$delivery_item->delivery_detail_product_hsn_code}}<br>{{number_format($delivery_item->delivery_detail_price,2)}}<br>{{number_format($delivery_item->delivery_detail_amount,2)}}
                  </td>
                  <td  align="left"><br>{{$delivery_item->delivery_detail_quantity}} {{$delivery_item->delivery_detail_unit}}<br>{{number_format($delivery_item->delivery_detail_discount_amount,2)}}
                  </td>
               </tr>
               @endforeach
               <tr>
                  <td align="left" colspan="2"><b>{{ trans('lang.taxable_amount') }}</b></td>
                  <td align="right" ><b>{{number_format($delivery_challan->delivery_taxable_amount,2)}}</b></td>
               </tr>
                 <?php if($delivery_challan->delivery_igst_amount!=0){ ?>
               <tr>
                  <td align="left"  colspan="2">{{ trans('lang.po_sgst_amount') }}</td>
                  <td align="right" >{{number_format($delivery_challan->delivery_sgst_amount,2)}}</td>
               </tr>
                <tr>
                  <td align="left"  colspan="2">{{ trans('lang.po_cgst_amount') }}</td>
                  <td align="right" >{{number_format($delivery_challan->delivery_cgst_amount,2)}}</td>
               </tr>
                <?php } ?> 
                 @if($delivery_challan->delivery_additional_charge>0)    
               <tr>
                  <td align="left" colspan="2" class="border_none">{{$delivery_challan->delivery_additional_charge_description}}</td>
                  <td align="right" class="border_none">{{number_format($delivery_challan->delivery_additional_charge,2)}}</td>
               </tr>
                @endif
               <tr>
                  <td align="left"  colspan="2" class="border_none">{{ trans('lang.po_round_off') }}</td>
                  <td align="right" class="border_none">{{number_format($delivery_challan->delivery_round_off,2)}}</td>
               </tr>
               <tr>
                  <td align="left"  colspan="2" class="border_none"><b>{{ trans('lang.po_table_grand_total') }}</b></td>
                  <td align="right" class="border_none"><b>{{number_format($delivery_challan->delivery_total_amount,2)}}</b></td>
               </tr>
            </table>
            <br>
            <p></p>
