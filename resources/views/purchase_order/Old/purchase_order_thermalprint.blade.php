            ...
            <table  cellpadding="5" style="font-size:14px;line-height: 18px;">
               <tbody>
                  <tr>
                     <td>
                        ...
                        {{ trans('lang.po_date') }}: {{date('d-m-Y',strtotime($purchase_order->purchase_date))}}<br>
                        {{ trans('lang.po_no') }}: {{$purchase_order->purchase_code}}<br>
             ...
             <table class="thermal_list_2inch_original"  cellpadding="5">
               ...
               <tr>
                  ...
                  <td align="right" ><b>{{number_format($purchase_order->purchase_taxable_amount,2)}}</b></td>
               </tr>
                 <?php if($purchase_order->purchase_igst_amount!=0){ ?>
               <tr>
                  <td align="left"  colspan="2">{{ trans('lang.po_sgst_amount') }}</td>
                  <td align="right" >{{number_format($purchase_order->purchase_sgst_amount,2)}}</td>
               </tr>
                <tr>
                  <td align="left"  colspan="2">{{ trans('lang.po_cgst_amount') }}</td>
                  <td align="right" >{{number_format($purchase_order->purchase_cgst_amount,2)}}</td>
               </tr>
                <?php } ?> 
                 @if($purchase_order->purchase_additional_charge>0)    
               <tr>
                  <td align="left" colspan="2" class="border_none">{{$purchase_order->purchase_additional_charge_description}}</td>
                  <td align="right" class="border_none">{{number_format($purchase_order->purchase_additional_charge,2)}}</td>
               </tr>
                @endif
               <tr>
                  <td align="left"  colspan="2" class="border_none">{{ trans('lang.po_round_off') }}</td>
                  <td align="right" class="border_none">{{number_format($purchase_order->purchase_round_off,2)}}</td>
               </tr>
               <tr>
                  <td align="left"  colspan="2" class="border_none"><b>{{ trans('lang.po_table_grand_total') }}</b></td>
                  <td align="right" class="border_none"><b>{{number_format($purchase_order->purchase_total_amount,2)}}</b></td>
               </tr>
            </table>
            <br>
            <p></p>
