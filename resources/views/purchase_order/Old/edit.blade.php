    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          ...
          <li class="breadcrumb-item active">{{trans('lang.purchase_order_edit')}}</li>
        </ol>
      </div><!-- /.col -->
    </div><!-- /.row -->
  </div><!-- /.container-fluid -->
  ...
  <div class="card">
    <div class="card-header">
      <ul class="nav nav-tabs align-items-end card-header-tabs w-100">
        ...
        <li class="nav-item">
          <a class="nav-link" id="new-invoice" href="{!! route('purchaseOrder.create') !!}"><i class="fa fa-plus mr-2"></i>{{trans('lang.purchase_order_create')}}</a>
        </li>
        @endcan
        <li class="nav-item">
          <a class="nav-link active" href="{!! url()->current() !!}"><i class="fa fa-pencil mr-2"></i>{{trans('lang.purchase_order_edit')}}</a>
        </li>
      </ul>
    </div>
    
    <div class="card-body">
      {!! Form::model($purchase_order, ['route' => ['purchaseOrder.update', $purchase_order->id], 'method' => 'patch']) !!}
      <div class="row">
        @include('purchase_order.edit_fields')
      </div>
      {!! Form::close() !!}
      <div class="clearfix"></div>
    </div>
...
<script>
  ...
  function addItem() {
    var party = $('#purchase_party').val();
     if(party > 0) {
      ...
      $.ajax({
          url: "{{ url('/purchaseOrder/loadPurchaseProducts') }}",
          type: 'post',
          data: {
            '_token' : "{{ csrf_token() }}"
          }
  ...
  function addItembyBarcode() {
    var party = $('#purchase_party').val();
     if(party > 0) {
      $('#productModal').modal('show');
      $.ajax({
          url: "{{ url('/purchaseOrder/loadPurchaseProductsbyBarcode') }}",
          type: 'post',
          data: {
            '_token' : "{{ csrf_token() }}"
          }
  ...
  $("document").ready(function(){
      $("#productForm").submit(function(e){
          ...
          e.preventDefault();
          $.ajax({
              url: "{{ url('/purchaseOrder/loadPurchaseItems') }}",
              type: 'post',
              dataType: 'JSON',
