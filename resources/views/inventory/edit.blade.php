<div class="modal-header">
  <h4 class="modal-title" id="myModalLabel">Edit Product Inventory</h4>
...
{!! Form::model($inventory, ['route' => ['inventory.update', $inventory->id], 'method' => 'patch', 'class' => 'inventory-form']) !!}
  ...
  <div class="modal-body">
    ...
    <div class="row">
      <div class="col-md-12">  
        ...
        <div class="form-group">
          {!! Form::label('date', 'Date', ['class' => ' control-label text-right required']) !!}
          {!! Form::text('date', date('d-m-Y'),  ['class' => 'form-control datepicker','readonly'=>'readonly']) !!}
        </div>

        <div class="form-group">
          {!! Form::label('type', 'Type', ['class' => ' control-label text-left required']) !!}
          {!! Form::select('type', ['add'=>'Add','reduce'=>'Reduce'], null, ['class' => 'form-control']) !!}
        </div>

        <div class="form-group usage-elem" style="display:none;">
          {!! Form::label('usage', 'Usage', ['class' => ' control-label text-left required']) !!}
          {!! Form::select('usage', ['normal'=>'Normal','wastage'=>'Wastage'], null, ['class' => 'form-control']) !!}
        </div>
        ...
        <!-- Image Field -->
        <div class="form-group">
            {!! Form::label('image', 'Product Image', ['class' => 'col-3 control-label text-left']) !!}
        ...
        <script type="text/javascript">
            ...
            @if(isset($inventory) && $inventory->hasMedia('image'))
            var15866134771240834480ble = {
                name: "{!! $inventory->getFirstMedia('image')->name !!}",
                size: "{!! $inventory->getFirstMedia('image')->size !!}",
                type: "{!! $inventory->getFirstMedia('image')->mime_type !!}",
                collection_name: "{!! $inventory->getFirstMedia('image')->collection_name !!}"};
            @endif
            var dz_var15866134771240834480ble = $(".dropzone.image").dropzone({
                ...
                init: function () {
                @if(isset($inventory) && $inventory->hasMedia('image'))
                    dzInit(this,var15866134771240834480ble,'{!! url($inventory->getFirstMediaUrl('image','thumb')) !!}')
                @endif
                },
                ...
                removedfile: function (file) {
                    dzRemoveFile(
                        file, var15866134771240834480ble, '{!! url("inventory/remove-media") !!}',
                        'image', '{!! isset($inventory) ? $inventory->id : 0 !!}', '{!! url("uplaods/clear") !!}', '{!! csrf_token() !!}'
                    );
                }
            });
