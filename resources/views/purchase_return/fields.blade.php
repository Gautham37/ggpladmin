    ...
    <div class="row" style="margin-top:20px;">
      ...
      @include('layouts.invoice_header')
        ...
        <div class="col-md-4 mt-4">
          ...
          <div class="form-group row">
            {!! Form::label('code', 'Purchase Return No', ['class' => 'label-left col-md-4 control-label text-left']) !!}
          ...
          <div class="form-group row">
            ...
            {!! Form::text('date', isset($purchase_return) ? $purchase_return->date->format('d-m-Y') : date('d-m-Y'),  ['class' => 'col-md-8 form-control datepicker', 'readonly' => 'readonly']) !!}
          </div>

          <div class="form-group row">
            {!! Form::label('valid_date', 'Valid Date', ['class' => 'label-left col-md-4 control-label text-left']) !!}
            {!! Form::text('valid_date', isset($purchase_return) ? $purchase_return->valid_date->format('d-m-Y') : date('d-m-Y', time() + (86400 * 15)),  ['class' => 'col-md-8 form-control datepicker', 'readonly' => 'readonly']) !!}
          </div>
      ...
      <div class="col-md-12">
          <table id="salesItems" class="table table-bordered tbl-product-details">
            ...
            <tbody class="purchase-return-items"></tbody>
            <!-- <tfoot>
    ...
    <div class="row">  
      ...
      <div class="col-md-7">
        ...
        <div class="col-md-12"> 
          ...
          {!! Form::textarea('terms_and_conditions', isset($purchase_return) ? null : setting('app_invoice_terms_and_conditions'), ['class' => 'form-control']) !!}
        </div>
      </div>
...
<div class="form-group col-12 text-right">
  <button type="submit" class="btn btn-{{setting('theme_color')}} purchase-return-form-submit" ><i class="fa fa-save"></i> Save Purchase Return</button>
  <a href="{!! route('purchaseReturn.index') !!}" class="btn btn-default"><i class="fa fa-undo"></i> Cancel </a>
</div>
...
<script>
  ...
  function cartarrayToTable() {
    ...
    $(".purchase-return-items").html(in_html);
  ...
  @if(isset($purchase_return))
    ...
    $(document).ready(function() {
      ...
      var url   = '{!!route('purchaseReturn.show',$purchase_return->id)!!}';
      var token = "{{ csrf_token() }}";
      $.ajax({
          type: 'GET',
          data: {
  ...
  @if(!isset($purchase_invoice) && isset($_GET['purchase_invoice_id']) && $_GET['purchase_invoice_id'] > 0)
  
    $(document).ready(function() {

      var url   = '{!!route('purchaseInvoice.show',$_GET['purchase_invoice_id'])!!}';
      var token = "{{ csrf_token() }}";
      $.ajax({
          type: 'GET',
          data: {
          ...
          url: url,
          success: function (response) {      
            if(response.data.amount_due == 0) { 

              $('.extra_items').append('<input type="hidden" name="purchase_invoice_id" value="'+response.data.id+'" />');
