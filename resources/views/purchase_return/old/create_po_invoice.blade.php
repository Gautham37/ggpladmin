...
<div class="content">
  ...
  <div class="card">
    ...
    <div class="card-body">
      ...
      <div class="row">
        <input type="hidden" name="purchase_order_id" value="{{$purchase_invoice->id}}">
        @include('purchase_invoice.edit_fields')
      </div>
      {!! Form::close() !!}
      <div class="clearfix"></div>
    </div>
...
<script>
  ...
  $("document").ready(function(){
      $.ajax({
          url: "{{ url('/purchaseOrder/loadPurchaseDetailItems') }}",
          type: 'post',
          dataType: 'JSON',
          data: {
            '_token' : "{{ csrf_token() }}",
  ...
  function getPartyDetail(party) {
      ...
      .done(function(response) {
          ...
          $('.purchase_party_details').html('<p> <strong>Address :</strong> '+data.address+' <br> <strong>Phone :</strong> '+data.phone+' , '+data.mobile+'</p>');
      })
      .fail(function(response) {
          console.log(response);
      });
