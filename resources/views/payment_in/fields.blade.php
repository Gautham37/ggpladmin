<div class="col-md-12 column custom-from-css">
  <div class="row">
    ...
    <div class="col-md-3">
      <div class="form-group">
        {!! Form::label('code', 'Payment In Code',['class' => ' control-label text-left']) !!}
        {!! Form::text('code', isset($payment_in_no) ? $payment_in_no : null ,  ['class' => 'form-control']) !!}
      </div>
    </div>
...
<div class="form-group col-12 text-right">
  <button type="submit" class="btn btn-{{setting('theme_color')}} payment-in-form-submit" ><i class="fa fa-save"></i> Save Payment In</button>
  <a href="{!! route('paymentIn.index') !!}" class="btn btn-default"><i class="fa fa-undo"></i> Cancel</a>
</div>
...
<script>
    ...
    function arraytoTable() {
      ...
      $.each(invoices, function(key,value) { 
        ...
        if(value.payment_in_settle_id != undefined && value.payment_in_settle_id != ''){
            html += '<input type="hidden" name="payment_in_settle_id[]" value="'+value.payment_in_settle_id+'" />';   
        }
      ...
      $.ajax({
          ...
          success: function (response) {
            if(response.data != null) {
              ...
              if(response.data.pendingsalesinvoices != null) {
                if(response.data.pendingsalesinvoices.length > 0) {
                  $.each(response.data.pendingsalesinvoices, function(key,value) {

                    var invoice_obj = {};
                    invoice_obj  = value;
                    invoice_obj.column_name   = 'sales_invoice_id';
              ...
              if(response.data.pendingpurchasereturns != null) {
                if(response.data.pendingpurchasereturns.length > 0) {
                  $.each(response.data.pendingpurchasereturns, function(key,value) {

                    var invoice_obj = {};
                    invoice_obj  = value;
                    invoice_obj.column_name   = 'purchase_return_id';
  ...
  @if(isset($payment_in))
    ...
    $(document).ready(function() {
      ...
      var url   = '{!!route('paymentIn.show',$payment_in->id)!!}';
      var token = "{{ csrf_token() }}";
      $.ajax({
          type: 'GET',
          data: {
          ...
          success: function (response) {      
               ...
               if(response.data.paymentinsettle.length > 0){
                  $.each(response.data.paymentinsettle, function(key,value){
                     
                    var invoice_obj = {};
                    if(value.settle_type=='sales') {
                    
                    invoice_obj                       = value.salesinvoice;
                    invoice_obj.amount_due            = parseFloat(value.salesinvoice.total) - (parseFloat(value.salesinvoice.total) - parseFloat(value.amount));
                      
                    } else if(value.settle_type=='purchase') {

                    invoice_obj                       = value.purchasereturn;
                    invoice_obj.amount_due            = parseFloat(value.purchasereturn.total) - (parseFloat(value.purchasereturn.total) - parseFloat(value.amount));  

                    }
                    
                    invoice_obj.payment_in_settle_id  = value.id;
