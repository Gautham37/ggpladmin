    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          ...
          <li class="breadcrumb-item active">{{trans('lang.purchase_invoice_edit')}}</li>
        </ol>
      </div><!-- /.col -->
    </div><!-- /.row -->
  </div><!-- /.container-fluid -->
  ...
  <div class="card">
    <div class="card-header">
      <ul class="nav nav-tabs align-items-end card-header-tabs w-100">
        ...
        <li class="nav-item">
          <a class="nav-link active" href="{!! url()->current() !!}"><i class="fa fa-plus mr-2"></i>{{trans('lang.purchase_invoice_create')}}</a>
        </li>
        @endcan
      </ul>
    </div>
    ...
    <div class="card-body">
      {!! Form::open(['route' => 'purchaseInvoice.store','id' => 'purchaseInvoiceForm']) !!}
      <div class="row">
        <input type="hidden" name="supplier_request_id" value="{{$purchase_invoice->id}}">
        @include('purchase_invoice.sr_fields')
      </div>
      {!! Form::close() !!}
      <div class="clearfix"></div>
    </div>
...
<script>
  ...
  function addItem() {
    ...
    if(party > 0) {
      ...
      $.ajax({
          url: "{{ url('/purchaseInvoice/loadPurchaseProducts') }}",
          type: 'post',
          data: {
            'party'  : party,
            '_token' : "{{ csrf_token() }}"
  ...
  function addItembyBarcode() {
    ...
    if(party > 0) {
      $('#productModal').modal('show');
      $.ajax({
          url: "{{ url('/purchaseInvoice/loadPurchaseProductsbyBarcode') }}",
          type: 'post',
          data: {
            'party'  : party,
            '_token' : "{{ csrf_token() }}"
      ...
      .done(function(results) {
          ...
          $.each(results, function (key,val) { 
              ...
              $.each(val.tax_rates, function (keys,vals) {
                  if(val.sr_detail_tax_percent == vals.rate) { var status='selected'; } else { var status=''; }
                  tax_options+='<option '+status+' value="'+vals.rate+'">'+vals.name+'</option>';
              });

              $('#purchaseItems tr:last').after('<tr> <input type="hidden" name="purchase_detail_id[]" value="'+val.id+'"> <input type="hidden" name="product_id[]" value="'+val.sr_detail_product_id+'"> <input type="hidden" name="product_unit[]" value="'+val.sr_detail_unit+'"><td>'+val.s_no+'</td><td><input type="text" name="product_name[]" class="form-control" value="'+val.sr_detail_product_name+'"></td><td><input type="text" name="product_hsn[]" class="form-control" value="'+val.sr_detail_product_hsn_code+'"></td><td><input type="text" name="product_mrp[]" class="form-control" value="'+(val.sr_detail_mrp).toFixed(2)+'"></td><td class="inline-flex"><input type="text" name="product_quantity[]" class="form-control product_quantity" value="'+val.sr_detail_quantity+'"> <span class="item-unit">'+val.sr_detail_unit+'</span></td><td><input type="text" name="product_price[]" class="form-control product_price" value="'+(val.sr_detail_price).toFixed(2)+'"></td><td><div class="input-group"><span class="input-group-addon">%</span> <input type="text" name="product_discount_percent[]" class="form-control product_discount_percent" value="0"></div><div class="input-group"><span class="input-group-addon">₹</span> <input type="text" name="product_discount_price[]" class="form-control product_discount_price" value="0"></div></td><td><div class="input-group"> <select name="product_tax[]" class="form-control product_tax"> '+tax_options+' </select><input type="hidden" class="product_sgst_per" name="product_sgst_per[]" value=""><input type="hidden" name="product_cgst_per[]" class="product_cgst_per" value=""><input type="hidden" class="product_sgst_pri" name="product_sgst_pri[]" value=""><input type="hidden" name="product_cgst_pri[]" class="product_cgst_pri" value=""></div></td><td><input type="text" name="amount[]" class="form-control product_subtotal" value=""></td><td><a onclick="removePurchaseeRow(this);" title="Remove"><i class="fa fa-trash"></i></a></td></tr>');
  ...
  $("document").ready(function(){
      $("#productForm").submit(function(e){
          e.preventDefault();
          $.ajax({
              url: "{{ url('/purchaseInvoice/loadPurchaseItems') }}",
              type: 'post',
              dataType: 'JSON',
  ...
  function getPartyDetail(party) {
      $.ajax({
          url: "{{ url('/purchaseInvoice/loadParty') }}",
          type: 'post',
          data: {
            '_token' : "{{ csrf_token() }}",
            'party'  : party.value
      ...
      .done(function(response) {
          ...
          $('.purchase_party_details').html('<p> <strong>Address :</strong><br> '+data.address_line_1+' '+data.address_line_2+' <br> '+data.city+' - '+data.pincode+', '+data.state+' <br> <strong>Phone :</strong> '+data.phone+' , '+data.mobile+'</p>');
      })
      .fail(function(response) {
          console.log(response);
      });
