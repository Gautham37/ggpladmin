    ...
    <div class="row" style="margin-top:20px;">
      ...
      @include('layouts.invoice_header')
        ...
        <div class="col-md-4 mt-4">
          ...
          <div class="form-group row">
            {!! Form::label('code', 'Purchase Invoice No', ['class' => 'label-left col-md-4 control-label text-left']) !!}
            {!! Form::text('code', isset($purchase_no) ? $purchase_no : null ,  ['class' => 'col-md-8 form-control']) !!}
          </div>

          <div class="form-group row">
            {!! Form::label('date', 'Date', ['class' => 'label-left col-md-4 control-label text-left']) !!}
            {!! Form::text('date', isset($purchase_invoice) ? $purchase_invoice->date->format('d-m-Y') : date('d-m-Y'),  ['class' => 'col-md-8 form-control datepicker', 'readonly' => 'readonly']) !!}
          </div>

          <div class="form-group row">
            {!! Form::label('valid_date', 'Valid Date', ['class' => 'label-left col-md-4 control-label text-left']) !!}
            {!! Form::text('valid_date', isset($purchase_invoice) ? $purchase_invoice->valid_date->format('d-m-Y') : date('d-m-Y', time() + (86400 * 15)),  ['class' => 'col-md-8 form-control datepicker', 'readonly' => 'readonly']) !!}
          </div>
      ...
      <div class="col-md-12">
          <table id="salesItems" class="table table-bordered tbl-product-details">
            ...
            <tbody class="purchase-invoice-items"></tbody>
            <!-- <tfoot>
    ...
    <div class="row">  
      ...
      <div class="col-md-7">
        ...
        <div class="col-md-12"> 
          ...
          {!! Form::textarea('terms_and_conditions', isset($purchase_invoice) ? null : setting('app_invoice_terms_and_conditions'), ['class' => 'form-control']) !!}
        </div>
      </div>
...
<div class="form-group col-12 text-right">
  <button type="submit" class="btn btn-{{setting('theme_color')}} purchase-invoice-form-submit" ><i class="fa fa-save"></i> Save Purchase Invoice</button>
  <a href="{!! route('purchaseInvoice.index') !!}" class="btn btn-default"><i class="fa fa-undo"></i> Cancel </a>
</div>
...
<script>
  ...
  function cartarrayToTable() {
    var in_html = '';
    var i     = 0;
    $(".purchase-invoice-items").html('');
    $.each(invoice_items, function(key,value) { 
      i++;

      var item_discount_amount = value.discount_amount == "" || isNaN(value.discount_amount) ? 0 : parseFloat(value.discount_amount);
      var item_tax             = value.tax == "" || isNaN(value.tax) ? 0 : parseFloat(value.tax);

      if(parseFloat(value.quantity) > 0 && parseFloat(value.unit_price) > 0) {         
          if(value.tax_type == 'exclusive'){             
             total_amount   = parseFloat(value.unit_price) * parseFloat(value.quantity) - parseFloat(item_discount_amount);
             gst_amount     = parseFloat(total_amount) / 100 * parseFloat(item_tax);
             amount         = parseFloat(total_amount) + parseFloat(gst_amount); 
          } else {             
             amount     = parseFloat(value.unit_price) * parseFloat(value.quantity) - parseFloat(item_discount_amount);
             gst_amount = 0;
             value.tax  = 0;
          }
      } else {
    ...
    $(".purchase-invoice-items").html(in_html);
         ...
         iziToast.show({
             ...
             buttons: [
                 ['<button>Yes</button>', function (instance, toast) {
                     ...
                     $('.delete_items').append('<input type="hidden" name="delete_item_id[]" value="'+invoice_items[index].invoice_item_id+'" />');
  ...
  @if(isset($purchase_invoice))
    ...
    $(document).ready(function() {
      ...
      var url   = '{!!route('purchaseInvoice.show',$purchase_invoice->id)!!}';
      var token = "{{ csrf_token() }}";
      $.ajax({
          type: 'GET',
          data: {
  ...
  @if(!isset($purchase_invoice) && isset($_GET['purchase_order_id']) && $_GET['purchase_order_id'] > 0)
  
    $(document).ready(function() {

      var url   = '{!!route('purchaseOrder.show',$_GET['purchase_order_id'])!!}';
      var token = "{{ csrf_token() }}";
      $.ajax({
          type: 'GET',
          data: {
          ...
          success: function (response) {      
            if(response.data.status!='invoiced') {     
              $('.extra_items').append('<input type="hidden" name="purchase_order_id" value="'+response.data.id+'" />');
              $('.market_id').val(response.data.market_id);
              $('.market_id').trigger('change');
              if(response.data.items.length > 0){
                  $.each(response.data.items, function(key,value){
  ...
  @if(!isset($purchase_invoice) && isset($_GET['vendor_stock_id']) && $_GET['vendor_stock_id'] > 0)
  
    $(document).ready(function() {

      var url   = '{!!route('vendorStock.show',$_GET['vendor_stock_id'])!!}';
      var token = "{{ csrf_token() }}";
      $.ajax({
          type: 'GET',
          data: {
          ...
          url: url,
          success: function (response) {      
            if(response.data.status!='invoiced' && response.data.status=='approved') { 

              $('.extra_items').append('<input type="hidden" name="vendor_stock_id" value="'+response.data.id+'" />');
              $('.market_id').val(response.data.market_id);
              $('.market_id').trigger('change');
              ...
              if(response.data.items.length > 0){
                  $.each(response.data.items, function(key,value){
                    ...
                    invoice_item_obj.tax              = 0;
                    invoice_item_obj.tax_type         = '';
