...
<div class="content-header">
  <div class="container-fluid">
    <div class="row mb-2">
      <div class="col-sm-6">
        <h1 class="m-0 text-dark">Payment Out<small class="ml-3 mr-3">|</small><small>Payment Out Management</small></h1>
      </div><!-- /.col -->
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          <li class="breadcrumb-item"><a href="{{url('/dashboard')}}"><i class="fa fa-dashboard"></i> Dashboard </a></li>
          <li class="breadcrumb-item"><a href="{!! route('paymentOut.index') !!}">Payment Out List </a>
          </li>
  ...
  <div class="card">
    <div class="card-header d-print-none">
      <ul class="nav nav-tabs align-items-end card-header-tabs w-100">
        <li class="nav-item">
          <a class="nav-link" href="{!! route('paymentOut.index') !!}"><i class="fa fa-list mr-2"></i>Payment Out List</a>
        </li>
        <li class="nav-item">
          <a class="nav-link active" href="{!! url()->current() !!}"><i class="fa fa-plus mr-2"></i>Payment Out</a>
        </li>
      </ul>
      
        <div class="dropdown float-right top-action-btn">

        @can('paymentOut.print')
        <div class="dropdown float-right top-action-btn">
          ...
          <ul class="dropdown-menu">
            <li>
              <a target="_blank" id="print-invoice" href='{{route("paymentOut.print",[$payment_out->id,"1","download"])}}'>
                Download Original
              </a>
            </li>
            <li>
              <a target="_blank" id="print-invoice" href='{{route("paymentOut.print",[$payment_out->id,"2","download"])}}'>
                Download Duplicate
              </a>
            </li>
            <li>
        ...
        <div class="dropdown float-right top-action-btn">
          ...
          <ul class="dropdown-menu">
            <li>
              <a target="_blank" id="print-invoice" href='{{route("paymentOut.print",[$payment_out->id,"1","print"])}}'>
                Print Original
              </a>
            </li>
            <li>
              <a target="_blank" id="print-invoice" href='{{route("paymentOut.print",[$payment_out->id,"2","print"])}}'>
                Print Duplicate
              </a>
            </li>
            <li>
        ...
        <!-- <div class="float-right top-action-btn">
            <button style="background: #c30052;border: 1px solid #c30052;" id="thermal-print-invoice" class="btn btn-primary btn-sm thermal_prints" href='{{route("paymentOut.print",[$payment_out->id,"1","thermalprint"])}}'>
              Thermal Print
            </button>
        </div> -->
    ...
    <div class="card-body">
            ...
            <tbody>
                ...
                <tr>
                  <td width="60%" colspan="6" rowspan="2">
                    ...
                    <h6><a href="{{route('markets.view',$payment_out->market->id)}}"><b>{{$payment_out->market->name}}</b></a></h6>
                    <p>
                    <b>Address : </b> 
                  ...
                  <td width="20%" colspan="2">
                    <p><b>Payment Out No</b></p>
                    <p>{{$payment_out->code}}</p>  
                  </td>
                  <td width="20%" colspan="2">
                    <p><b>Date</b></p>
                    <p>{{$payment_out->date->format('d-m-Y')}}</p>
                  </td>
                </tr>
                ...
                <tr>
                  <td width="20%" colspan="2">
                    <p><b>Created By</b></p>
                    <p>{{$payment_out->createdby->name}}</p>  
                  </td>
                  <td width="20%" colspan="2">
                    <p><b>Payment Method</b></p>
                    <p>{{$payment_out->paymentmethod->name}}</p>  
                  </td>
                </tr>
                  ...
                  <tr>
                    <td width="60%">
                      @if($invoice->settle_type=='sales')
                        <a href="{{route('salesReturn.show',$invoice->salesreturn->id)}}">{{$invoice->salesreturn->code}}</a>
                      @elseif($invoice->settle_type=='purchase')
                        <a href="{{route('purchaseInvoice.show',$invoice->purchaseinvoice->id)}}">{{$invoice->purchaseinvoice->code}}</a>
                      @endif  
                    </td>
              ...
              @endif     
                ...
                <tr>
                  <td>
                    ...
                    <h6><b>Notes</b></h6>
                    <p>{!!$payment_out->notes!!}</p>

                  </td>
                  <td class="font-weight-600 text-right"> <h6>Total</h6> </td>
                  <td class="text-right">
                    <h6>{{setting('default_currency')}} {{number_format($payment_out->total,2,'.','')}}</h6>
                  </td>
                </tr>
            </tbody>
          </table>
      ...
      <div class="row d-print-none">
        ...
        <div class="form-group col-12 text-right">
          <a href="{!! route('paymentOut.index') !!}" class="btn btn-default"><i class="fa fa-undo"></i> Back </a>
        </div>
      </div>
