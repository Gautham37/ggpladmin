    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          ...
          <li class="breadcrumb-item active">{{trans('lang.supplier_request_create')}}</li>
        </ol>
      </div><!-- /.col -->
    </div><!-- /.row -->
  </div><!-- /.container-fluid -->
  ...
  <div class="card">
    <div class="card-header">
      <ul class="nav nav-tabs align-items-end card-header-tabs w-100">
        ...
        @endcan
        <li class="nav-item">
          <a class="nav-link active" href="{!! url()->current() !!}"><i class="fa fa-plus mr-2"></i>{{trans('lang.supplier_request_create')}}</a>
        </li>
      </ul>
    </div>
    <div class="card-body">
      {!! Form::open(['route' => 'supplierRequest.store','id' => 'supplierRequestForm']) !!}
      <div class="row">
        @include('supplier_request.fields')
      </div>
      {!! Form::close() !!}
      <div class="clearfix"></div>
    </div>
...
/*                  $('#salesItems tr:last').after('<tr> <input type="hidden" name="product_id[]" value="'+val.id+'"><input type="hidden" name="product_unit[]" value="'+val.unit+'"><td>'+val.s_no+'</td><td><input type="text" name="product_name[]" class="form-control" value="'+val.name+'"></td><td><input type="text" name="product_hsn[]" class="form-control" value="'+val.hsn_code+'"></td><td><input type="text" name="product_mrp[]" class="form-control" value="'+(val.purchase_price).toFixed(2)+'"></td><td class="inline-flex"><input type="text" name="product_quantity[]" class="form-control product_quantity" value="'+val.quantity+'"> <span class="item-unit">'+val.unit+'</span></td><a onclick="removesaleseRow(this);" title="Remove"><i class="fa fa-trash"></i></a></td></tr>');*/
