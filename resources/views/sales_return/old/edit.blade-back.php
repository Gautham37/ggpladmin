    ...
    <div class="row mb-2">
      ...
      <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
          ...
          <li class="breadcrumb-item active">{{trans('lang.sales_return_edit')}}</li>
        </ol>
      </div><!-- /.col -->
    </div><!-- /.row -->
  </div><!-- /.container-fluid -->
  ...
  <div class="card">
    <div class="card-header">
      <ul class="nav nav-tabs align-items-end card-header-tabs w-100">
        ...
        @endcan
        @can('salesReturn.create')
        <li class="nav-item">
          <a class="nav-link" href="{!! route('salesReturn.create') !!}"><i class="fa fa-plus mr-2"></i>{{trans('lang.sales_return_create')}}</a>
        </li>
        @endcan
        <li class="nav-item">
          <a class="nav-link active" href="{!! url()->current() !!}"><i class="fa fa-pencil mr-2"></i>{{trans('lang.sales_return_edit')}}</a>
        </li>
      </ul>
    </div>
    
    <div class="card-body">
      {!! Form::model($sales_return, ['route' => ['salesReturn.update', $sales_return->id], 'method' => 'patch', 'id' => 'salesReturnForm']) !!}
      <div class="row">
        @include('sales_return.edit_fields')
      </div>
      {!! Form::close() !!}
      <div class="clearfix"></div>
    </div>
...
<script>
  ...
  $("document").ready(function(){
      $.ajax({
          url: "{{ url('/salesReturn/loadsalesDetailItems') }}",
          type: 'post',
          dataType: 'JSON',
          data: {
            '_token' : "{{ csrf_token() }}",
            'sales_id' : "{{$sales_return->id}}"
          }
      })
      .done(function(results) {
          ...
          $.each(results, function (key,val) { 
              ...
              $.each(val.tax_rates, function (keys,vals) {
                  if(val.sales_detail_tax_percent == vals.rate) { var status='selected'; } else { var status=''; }
                  tax_options+='<option '+status+' value="'+vals.rate+'">'+vals.name+'</option>';
              });

              $('#salesItems tr:last').after('<tr> <input type="hidden" name="sales_detail_id[]" value="'+val.id+'"> <input type="hidden" name="product_id[]" value="'+val.sales_detail_product_id+'"> <input type="hidden" class="form-control product_unit" name="product_unit[]" value="'+val.sales_detail_unit+'"><td>'+val.s_no+'</td><td><input type="text" name="product_name[]" class="form-control" value="'+val.sales_detail_product_name+'"></td><td><input type="text" name="product_hsn[]" class="form-control" value="'+val.sales_detail_product_hsn_code+'"></td><td><input type="text" name="product_mrp[]" class="form-control" value="'+(val.sales_detail_mrp).toFixed(2)+'"></td><td class="inline-flex"><input type="text" name="product_quantity[]" class="form-control product_quantity" value="'+val.sales_detail_quantity+'"> <span class="item-unit">'+val.sales_detail_unit+'</span></td><td><input type="text" name="product_price[]" class="form-control product_price" value="'+(val.sales_detail_price).toFixed(2)+'"></td><td><div class="input-group"><span class="input-group-addon">%</span> <input type="text" name="product_discount_percent[]" class="form-control product_discount_percent" value="'+val.sales_detail_discount_percent+'"></div><div class="input-group"><span class="input-group-addon">₹</span> <input type="text" name="product_discount_price[]" class="form-control product_discount_price" value="'+val.sales_detail_discount_amount+'"></div></td><td><div class="input-group"> <select name="product_tax[]" class="form-control product_tax"> '+tax_options+' </select><input type="hidden" class="product_sgst_per" name="product_sgst_per[]" value="'+val.sales_detail_sgst_percent+'"><input type="hidden" name="product_cgst_per[]" class="product_cgst_per" value="'+val.sales_detail_cgst_percent+'"><input type="hidden" class="product_sgst_pri" name="product_sgst_pri[]" value="'+val.sales_detail_sgst+'"><input type="hidden" name="product_cgst_pri[]" class="product_cgst_pri" value="'+val.sales_detail_cgst+'"></div></td><td><input type="text" name="amount[]" class="form-control product_subtotal" value="'+val.sales_detail_amount+'"></td><td><a onclick="removeEditRow('+val.id+',this);" title="Remove"><i class="fa fa-trash"></i></a></td></tr>');
          });
          totalProducts();
