    ...
    <div class="row" style="margin-top:20px;">
      ...
      @include('layouts.invoice_header')
        ...
        <div class="col-md-4 mt-4">
          ...
          <div class="form-group row">
            {!! Form::label('code', 'Sales Return No', ['class' => 'label-left col-md-4 control-label text-left']) !!}
            {!! Form::text('code', isset($return_no) ? $return_no : null ,  ['class' => 'col-md-8 form-control']) !!}
          </div>

          <div class="form-group row">
            {!! Form::label('date', 'Date', ['class' => 'label-left col-md-4 control-label text-left']) !!}
            {!! Form::text('date', isset($sales_return) ? $sales_return->date->format('d-m-Y') : date('d-m-Y'),  ['class' => 'col-md-8 form-control datepicker', 'readonly' => 'readonly']) !!}
          </div>

          <div class="form-group row">
            {!! Form::label('valid_date', 'Valid Date', ['class' => 'label-left col-md-4 control-label text-left']) !!}
            {!! Form::text('valid_date', isset($sales_return) ? $sales_return->valid_date->format('d-m-Y') : date('d-m-Y', time() + (86400 * 15)),  ['class' => 'col-md-8 form-control datepicker', 'readonly' => 'readonly']) !!}
          </div>
      ...
      <div class="col-md-12">
          <table id="salesItems" class="table table-bordered tbl-product-details">
            ...
            <tbody class="sales-return-items"></tbody>
            <!-- <tfoot>
    ...
    <div class="row">  
      ...
      <div class="col-md-7">
        ...
        <div class="col-md-12"> 
          ...
          {!! Form::textarea('terms_and_conditions', isset($sales_return) ? null : setting('app_invoice_terms_and_conditions'), ['class' => 'form-control']) !!}
        </div>
      </div>
      ...
      <div class="col-md-5">
          <table class="table table-bordered">
            ...
            <tr>
              <td class="text-right">Pickup Charge</td>
              <td class="text-right">
                  <div class="input-group">
                    <span class="input-group-addon">{{setting('default_currency')}}</span>
...
<div class="form-group col-12 text-right">
  <button type="submit" class="btn btn-{{setting('theme_color')}} sales-return-form-submit" ><i class="fa fa-save"></i> Save Sales Return</button>
  <a href="{!! route('salesReturn.index') !!}" class="btn btn-default"><i class="fa fa-undo"></i> Cancel </a>
</div>
...
<script>
  ...
  function cartarrayToTable() {
    ...
    $.each(invoice_items, function(key,value) { 
      ...
      if(value.invoice_item_id != undefined && value.invoice_item_id != ''){
          in_html += '<input type="hidden" name="return_item_id[]" value="'+value.invoice_item_id+'" />';   
      }
    ...
    $(".sales-return-items").html(in_html);
  ...
  @if(isset($sales_return))
    ...
    $(document).ready(function() {
      ...
      var url   = '{!!route('salesReturn.show',$sales_return->id)!!}';
      var token = "{{ csrf_token() }}";
      $.ajax({
          type: 'GET',
          data: {
  ...
  @if(!isset($sales_invoice) && isset($_GET['sales_invoice_id']) && $_GET['sales_invoice_id'] > 0)
  
    $(document).ready(function() {

      var url   = '{!!route('salesInvoice.show',$_GET['sales_invoice_id'])!!}';
      var token = "{{ csrf_token() }}";
      $.ajax({
          type: 'GET',
          data: {
          ...
          url: url,
          success: function (response) {      
            if(response.data.amount_due == 0 && response.data.salesreturn == null) { 

              $('.extra_items').append('<input type="hidden" name="sales_invoice_id" value="'+response.data.id+'" />');
